...
public final class TestResponseMapper {
    ...
    public static List<ScreenshotResponse> buildScreenshotResponses(TestEntity entity) {
        return entity.getScreenshots()
                     .stream()
                     .map(TestResponseMapper::buildScreenshotResponse)
                     .toList();
    }

    private static ScreenshotResponse buildScreenshotResponse(TestScreenshotEntity testScreenshotEntity) {
    ...
    private static TestResponse buildTestResponse(TestEntity entity) {
        ...
        return TestResponse.builder()
                           ...
                           .configurationTestIdentifiers(configurationTest.getConfigurationIdentifiers() != null ? configurationTest.getConfigurationIdentifiers().stream().map(ConfigurationTestIdentifierEntity::getIdentifier).toList() : null)
    ...
    public static List<TestVariableResponse> buildTestVariableResponses(Map<String, String> entities) {
        var testVariableResponses = new ArrayList<TestVariableResponse>();
        entities.forEach((a, b) -> testVariableResponses.add(TestVariableResponse.builder().name(a).value(b).build()));
        return testVariableResponses;
    }

    public static List<TestResponse> buildTestResponses(List<TestEntity> entities) {
        return entities.stream()
                       .map(TestResponseMapper::buildTestResponse)
                       .toList();
    }
}
