...
public class ConfigurationSuiteEntity extends PanacheEntityBase {
    ...
    @ManyToOne
    @JoinColumn(name = "parent_configuration_suite_id", foreignKey = @ForeignKey(name = "fk__configuration_suite__parent_configuration_suite_id"))
    private ConfigurationSuiteEntity parentSuite;

    @Fetch(FetchMode.SUBSELECT)
    @OneToMany(mappedBy = "parentSuite", fetch = FetchType.EAGER, cascade = CascadeType.ALL, orphanRemoval = true)
    private List<ConfigurationSuiteEntity> subSuites;

    @Fetch(FetchMode.SUBSELECT)
    @OneToMany(mappedBy = "configurationSuite", fetch = FetchType.LAZY, cascade = CascadeType.ALL, orphanRemoval = true)
    @OrderBy("id ASC")
