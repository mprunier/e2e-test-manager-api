...
@ApplicationScoped
public class EnvironmentSynchronizationErrorRepository implements PanacheRepositoryBase<EnvironmentSynchronizationErrorEntity, Long> {

    public Optional<EnvironmentSynchronizationErrorEntity> findByEnvironmentIdAndFile(
            long environmentId,
            String file) {
        return find("environment.id = ?1 AND file = ?2", environmentId, file)
                .stream()
                .findFirst();
    }

    public void deleteByEnvironmentIdAndFile(
            long environmentId,
            String file) {
