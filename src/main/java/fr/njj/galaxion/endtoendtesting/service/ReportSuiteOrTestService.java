...
public class ReportSuiteOrTestService {
                ...
                report.getResults().forEach(result -> {
                    ...
                    if (file.equals(reportFile)) {
                        ...
                        if (fullTitle.toString().equals(configurationTest.getTitle())) {
                            mochaTestOptional = result.getTests()
                                                      .stream()
                                                      .filter(mochaTest -> fullTitle.toString().equals(mochaTest.getTitle()))
                                                      .findFirst();

                        } else {
                            mochaTestOptional = findTestInSuites(result.getSuites(), fullTitle.toString());
    ...
    private static ConfigurationStatus getStatus(MochaReportTestInternal mochaTest) {
        ...
        if (Boolean.TRUE.equals(mochaTest.getPending()) || Boolean.TRUE.equals(mochaTest.getSkipped())) {
    ...
    private static void createScreenshot(TestEntity test,
        ...
        buildTestTitleWithSuiteTitles(testTitleWithSuiteTitles, test.getConfigurationTest().getConfigurationSuite());
        ...
        for (var entry : screenshots.entrySet()) {
            ...
            if (testTitleWithSuiteTitles.size() != 1) {
                isTestScreenshot = testTitleWithSuiteTitles.stream().allMatch(suiteTitle -> entry.getKey().contains(suiteTitle));
            } else {
                isTestScreenshot = entry.getKey().contains(test.getConfigurationTest().getTitle()) && !entry.getKey().contains(" -- ");
            }
            if (isTestScreenshot) {
                TestScreenshotEntity
                        .builder()
                        .test(test)
