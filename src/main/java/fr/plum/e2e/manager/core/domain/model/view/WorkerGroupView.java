...
@Builder
public record WorkerGroupView(UUID id, String createdBy, ZonedDateTime createdAt, WorkerType type) {

  public static WorkerGroupView from(Worker group) {
    return builder()
  ...
  public static List<WorkerGroupView> findForSuite(
      String fileName, UUID suiteId, List<Worker> workers) {
    return workers.stream()
        .filter(group -> hasWorkerForSuite(group, fileName, suiteId))
        .map(WorkerGroupView::from)
        .toList();
  }

  public static List<WorkerGroupView> findForTest(UUID testId, List<Worker> workers) {
    return workers.stream()
