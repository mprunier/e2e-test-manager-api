...
-- changeset mprunier:1.0.0-240-update_config_test_status_and_suite_function.sql endDelimiter:go
...
BEGIN
    ...
    ELSEIF last_test_status != 'NEW' THEN
    ...
    FROM (SELECT t.status
          ...
          WHERE ct.configuration_suite_id = config_suite_id
            AND t.status IN ('FAILED', 'CANCELED', 'SYSTEM_ERROR', 'NO_CORRESPONDING_TEST', 'NO_REPORT_ERROR', 'UNKNOWN')) AS FailedTests;
    ...
    FROM (SELECT t.status
          FROM test t
                   ...
                   JOIN configuration_test ct ON t.configuration_test_id = ct.id
          WHERE ct.configuration_suite_id = config_suite_id
            AND t.status = 'IN_PROGRESS') AS InProgressTests;
    ...
    FROM (SELECT t.status
          FROM test t
                   ...
                   JOIN configuration_test ct ON t.configuration_test_id = ct.id
          WHERE ct.configuration_suite_id = config_suite_id
            AND t.status = 'NEW') AS InProgressTests;

    IF failed_count = 0 AND in_progress_count = 0 AND new_count = 0 THEN
        UPDATE configuration_suite SET status = 'SUCCESS', updated_at = NOW(), last_played_at = NOW() WHERE id = config_suite_id;
    ELSIF failed_count > 0 AND in_progress_count = 0 AND new_count = 0 THEN
