...
BEGIN
    ...
    FROM (SELECT t.status
          ...
          WHERE ct.configuration_suite_id = config_suite_id
            AND t.status = 'SUCCESS') AS SucceededTests;
    ...
    FROM (SELECT t.status
          FROM test t
                   ...
                   JOIN configuration_test ct ON t.configuration_test_id = ct.id
          WHERE ct.configuration_suite_id = config_suite_id
            AND t.status IN ('FAILED', 'SYSTEM_ERROR', 'NO_CORRESPONDING_TEST', 'NO_REPORT_ERROR', 'UNKNOWN')) AS FailedTests;
    ...
    FROM (SELECT t.status
          FROM test t
                   ...
                   JOIN configuration_test ct ON t.configuration_test_id = ct.id
          WHERE ct.configuration_suite_id = config_suite_id
            AND t.status = 'SKIPPED') AS SkippedTests;
